document.addEventListener("DOMContentLoaded", function () {
  ...
  function runAnimationsWhenReady() {
    ...
    // --- Sponsor Benefits Cards Animation ---
    ...
    benefitCards.forEach((card) => observerCards.observe(card));

    // Animate title separately
    const benefitsTitle = document.querySelector(".benefits-title");
    if (benefitsTitle) {
      const rect = benefitsTitle.getBoundingClientRect();
      if (rect.top < window.innerHeight && rect.bottom > 0) {
        benefitsTitle.classList.add("animate-show");
      }
    }
  }
